...
unsigned long NrPacket = 10000;    //Set the number of send packets
char x = 0;
...
void setup() {
  ...
  radio.setDataRate(RF24_1MBPS);       //max 1Mbps if ACK ar disabled, setting 1MBPS and 250KBPS
...
void loop() {
  ...
  while(counter < NrPacket){   //Transmit data until the wanted number of packets are transmitted
    ...
    delayMicroseconds(200);       //Set a dalay to make sure the received can follow  
    counter++;
    stopTime = micros();
  }
